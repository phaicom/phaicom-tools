...
import type { PopoverProps } from "@/shared/components/ui/Popover";

import { dropdownItemStyles } from "@/shared/components/ui/ListBox";
        ...
        <>
          {selectionMode !== "none" && (
            <span className="flex w-4 items-center">
              {isSelected && <LuCheck aria-hidden className="h-4 w-4" />}
            </span>
          )}
          <span className="flex flex-1 items-center gap-2 truncate font-normal group-selected:font-semibold">
            {children}
          </span>
          {hasSubmenu && <LuChevronRight aria-hidden className="absolute right-2 h-4 w-4" />}
        </>
      ))}
    </AriaMenuItem>
  );
